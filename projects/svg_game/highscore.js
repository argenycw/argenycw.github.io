...
function getHighScoreTable() {
    ...
    for (var i = 0; i < 5; i++) {
		...
		table.push(new ScoreRecord(data[0], data[1]));
    }
    return table;
}
...
function addHighScore(record, node) {
	...
	name.setAttribute("dy", "60");
	if (record.name == player.name) name.setAttribute("style", "fill:orange");
	...
	if (record.name == player.name) score.setAttribute("style", "fill:orange");
